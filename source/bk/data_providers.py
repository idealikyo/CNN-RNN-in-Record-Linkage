...
class DataProvider(object):
    ...
    def __init__(self, inputs, targets, batch_size, max_num_batches=-1,
                 shuffle_order=True, rng=None, refers=None):
    ...
    def reset(self):
        ...
        self.refers = self.refers[inv_perm]
    ...
    def next(self):
        ...
        refers_batch = self.refers[batch_slice]
        targets_batch = self.targets[batch_slice]
        self._curr_batch += 1
        return inputs_batch, refers_batch, targets_batch
...
class MovieDataProvider(DataProvider):
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_path = os.path.join(
            os.getcwd(), '../dataset/', 'movie-{0}.npz'.format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
        loaded = np.load(data_path)
        inputs, refers, targets = loaded['inputs'], loaded['refers'], loaded['targets']
        inputs = inputs.astype(np.float32)
        refers = refers.astype(np.float32)
        # pass the loaded data to the parent class __init__
        super(MovieDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng, refers)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, refers_batch, targets_batch = super(MovieDataProvider, self).next()
        return inputs_batch, refers_batch, self.to_one_of_k(targets_batch)
...
class IndexDataProvider(DataProvider):
    """Data provider for Index data."""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, flatten=False):
        """Create a new Index data provider object.
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            os.environ['MLP_DATA_DIR'], 'index-{0}.npz'.format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
        ...
        inputs = inputs.astype(np.float32)
        if flatten:
            inputs = np.reshape(inputs, newshape=(-1, 6*20))
        else:
            inputs = np.reshape(inputs, newshape=(-1, 1, 6, 20))
        # pass the loaded data to the parent class __init__
        super(IndexDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(IndexDataProvider, self).next()
        return inputs_batch, self.to_one_of_k(targets_batch)

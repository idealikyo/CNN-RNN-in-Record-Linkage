...
import numpy as np
import os
from source import DEFAULT_SEED
...
class MovieDataProvider(DataProvider):
    ...
    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(MovieDataProvider, self).next()
...
class MovieDataLocalProvider(DataProvider):
    """Data provider for MNIST handwritten digit images."""
...
#         )
        # load data from compressed numpy file
#         loaded = np.load(data_path)
#         loaded = self.which_set
#         inputs, targets = loaded['inputs'], loaded['targets']
        inputs = inputs.astype(np.float32)
        # pass the loaded data to the parent class __init__
        super(MovieDataLocalProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(MovieDataLocalProvider, self).next()
        return inputs_batch, self.to_one_of_k(targets_batch)
